...
-- Performance indexes
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_posts_created_at_desc ON public.posts(created_at DESC);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_posts_author_created ON public.posts(author_id, created_at DESC);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_posts_category_created ON public.posts(category, created_at DESC);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_reactions_post_user ON public.reactions(post_id, user_id);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_reactions_user_created ON public.reactions(user_id, created_at DESC);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_messages_conversation ON public.direct_messages(sender_id, receiver_id, created_at DESC);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_messages_receiver_unread ON public.direct_messages(receiver_id, read_at) WHERE read_at IS NULL;
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_group_messages_group_time ON public.group_messages(group_id, created_at DESC);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_profiles_username_trgm ON public.profiles USING gin(username gin_trgm_ops);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_notifications_user_unread ON public.notifications(user_id, read_at) WHERE read_at IS NULL;
...
BEGIN
  ...
  -- Log the points award (could be in a separate audit table)
...
-- Storage policies would be created in the Supabase dashboard